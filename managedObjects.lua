...
function newObject( imageTable, width, height, x, y, frameSecondsTable, fps60, alpha, parentGroup, seconds )
	...
	local currentFrame; local drawCycle; local mSeconds; local totalDrarCycle
	local isFlipping
	...
	if (#frameSecondsTable==1) then
		-- Todos los frames duran lo mismo
		customTiming=false
	else
		customTiming=true
	end
	...
	if fps60 then
		mTotalSeconds = seconds*60
	else
		mTotalSeconds = seconds*30
	end
	
	-- Check to see if this object has an animation associated with it
	-- (depends on if secondsBetweenFrames paramater was set)
	if frameCount > 1 and frameSecondsTable then
		for i=1,#frameSecondsTable do
			if fps60 then
				mSecondsTable[i] = mCeil(frameSecondsTable[i] * 60)	--> get milliseconds based on paramaters
			else
				mSecondsTable[i] = mCeil(frameSecondsTable[i] * 30)
			end
		end
	
		mSeconds=mSecondsTable[1]
	...
	local enterFrameListener = function( self, event )
		if isActive then
			...
			if isAnimation and isAnimating and isActive then
				...
				if mTotalSeconds>0 and totalDrawCycle>mTotalSeconds then
					totalDrawCycle=1
					self:stopAnimation()
					self:showFrame(1)
				else
					if mTotalSeconds>0 then 
						totalDrawCycle = totalDrawCycle+1
					end
	...
	function object:nextFrame( shouldFlip )
		...
		if (customTiming) then
			mSeconds=mSecondsTable[theFrame]
		end
